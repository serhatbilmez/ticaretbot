...
exports.run = async (client, msg, args) => {
  ...
  if (arm.includes(args[0])) {
    ...
    if (!args[1]) {
      const embed = new Discord.RichEmbed()
        .setDescription(
          "Ayarlamak istediğiniz resmin linkini veya `sıfırla` yazınız!"
        )
  ...
  if (award.includes(args[0])) {
    if (!msg.member.hasPermission("MANAGE_ROLES")) {
      const embed = new Discord.RichEmbed()
        .setColor("RANDOM")
        .setDescription(
          "Bu komutu kullanabilmek için **Rolleri Yönet** iznine sahip olmalısın!"
        );
      msg.channel.send({ embed });
      return;
    }
    ...
    if (reset.includes(args[1])) {
      if (!db.has(`roll_${msg.guild.id}`)) {
        if (!db.has(`rollss_${msg.guild.id}`)) {
          const embed = new Discord.RichEmbed()
            .setDescription(
              "Rol ödülleri sistemi sıfırlanamıyor çünkü bu sistem aktif edilmemiş!"
            )
            .setColor("RANDOM");
          msg.channel.send({ embed });
          return;
    ...
    if (!args[2]) {
      const embed = new Discord.RichEmbed()
        .setDescription(
          `Lütfen **${r.name}** rolünü almak için kaçıncı seviyede olmak gerektiğini yazınız!`
        )
    ...
    if (isNaN(args[2])) {
      const embed = new Discord.RichEmbed()
        .setDescription(
          `Lütfen **${r.name}** rolünü almak için kaçıncı seviyede olmak gerektiğini sayı biçiminde yazınız!`
        )
        .setColor("RANDOM");
      msg.channel.send({ embed });
      return;
  ...
  if (saydam.includes(args[0])) {
    ...
    if (args[1] > 5 || args[1] < 1) {
      const embed = new Discord.RichEmbed()
        .setDescription(
          "Saydamlık derecesi için `5`ten fazla bir seçenek yok! \n**Dereceler:** `1`, `2`, `3`, `4`, `5`"
        )
        .setColor("RANDOM");
      msg.channel.send({ embed });
      return;
  ...
  let sira = "";
  const sorted = msg.guild.members
    .filter(u => !u.user.bot)
    .array()
    .sort((a, b) => {
      return (
        db.fetch(`seviye_${b.user.id + msg.guild.id}`) -
        db.fetch(`seviye_${a.user.id + msg.guild.id}`)
      );
    });
