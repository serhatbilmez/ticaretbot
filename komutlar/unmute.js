...
exports.run = (client, message, args) => {
  ...
  if(!modLog[message.guild.id]) return message.reply('Lütfen **.mod-log-ayarla** yazarak Moderasyon Kayıtları kanalı ayarlayınız!');
  if (!sRol[message.guild.id]) return message.reply('Lütfen **.sustur-rol-ayarla** yazarak Susturma Rolü ayarlayınız!');
  if (message.mentions.users.size < 1) return message.reply('Susturmasını kaldıracağın kişiyi etiketlemelisin! \n**Örnek:** .konuştur <@kişi-etiket>');
